...
import {
  GET_CAMPAIGN_FILTERING_PARAMETER, GET_CAMPAIGN_PERIOD_DATE
} from '@/modules/affiliate/getter-types'
import { UPDATE_CAMPAIGN_FILTERING_PARAMETER, UPDATE_CAMPAIGN_PERIOD_DATE } from '@/modules/affiliate/mutation-types'
...
export default {
  ...
  computed: {
    ...mapGetters({
      orderFiltering: GET_CAMPAIGN_FILTERING_PARAMETER,
      orderPeriodFiltering: GET_CAMPAIGN_PERIOD_DATE
    })
  },
  methods: {
    ...mapActions({
      updateOrderTypeFilter: UPDATE_CAMPAIGN_FILTERING_PARAMETER,
      updateOrderPeriodFilter: UPDATE_CAMPAIGN_PERIOD_DATE
    }),
    handleSelectFilters () {
      this.$router.back()
    },
    handleSelectCustomPeriod () {
      ...
      this.$router.push(`/affiliate/campaigns/${this.$route.params.name}/filter-date`)
    },
    goBack () {
      this.$router.back()
    }
