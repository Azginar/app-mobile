...
import {
  GET_REFERRAL_FILTERING_PARAMETER, GET_REFERRAL_PERIOD_DATE
} from '@/modules/affiliate/getter-types'
import { UPDATE_REFERRAL_FILTERING_PARAMETER, UPDATE_REFERRAL_PERIOD_DATE } from '@/modules/affiliate/mutation-types'
...
export default {
  ...
  computed: {
    ...mapGetters({
      orderFiltering: GET_REFERRAL_FILTERING_PARAMETER,
      orderPeriodFiltering: GET_REFERRAL_PERIOD_DATE
    })
  },
  methods: {
    ...mapActions({
      updateOrderTypeFilter: UPDATE_REFERRAL_FILTERING_PARAMETER,
      updateOrderPeriodFilter: UPDATE_REFERRAL_PERIOD_DATE
    }),
    handleSelectFilters () {
      this.$router.back()
    },
    handleSelectCustomPeriod () {
      ...
      this.$router.push(`/affiliate/referral/${this.$route.params.uid}/filter-date`)
    },
    goBack () {
      this.$router.back()
    }
